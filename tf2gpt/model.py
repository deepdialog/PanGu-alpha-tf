...
class Attention(tf.keras.layers.Layer):
    ...
    def call(self, x, kv_cache=None, query=None, **kwargs):
...
class Transformer(tf.keras.layers.Layer):
    ...
    def call(self, x, kv_cache=None, query=None, **kwargs):
        ...
        hidden_states = x
        layernorm_output = self.ln0(hidden_states)
        attn, cached_kv = self.attn(layernorm_output, query=query, kv_cache=kv_cache)
        residual = hidden_states
        layernorm_input = attn + residual

        # attn, cached_kv = self.attn(self.ln0(x), query=query, kv_cache=kv_cache)
        # x = x + attn
        
        layernorm_output = self.ln1(layernorm_input)
        mlp = self.intermediate_layer(layernorm_output)
        mlp = self.activation(mlp)
        mlp = self.output_layer(mlp)
        residual = layernorm_input
        x = mlp + residual

        # z = self.ln1(layernorm_input)
...
class PositionEmbedding(tf.keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 embedding_size, name="position_embeddings", **kwargs):
...
class GPT(tf.keras.Model):
    ...
    def call(self, x, kv_cache=None, use_cache=False, **kwargs):
        ...
        cached_kvs = []
        for i, layer in enumerate(self.transformers[:-1]):
            x, cached_kv = layer(
                x,
        ...
        # top query
        i = len(self.transformers) - 1
        layer = self.transformers[-1]
        x, cached_kv = layer(
            x,
            query=query,
            kv_cache=kv_cache[i] if kv_cache is not None else None)
